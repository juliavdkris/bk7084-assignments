...
class GridCell(Component):
    ...
    """
    def __init__(self, w=1.0, h=1.0, kind=GridCellKind.Empty, texture='assets/textures/grass.jpg'):
    ...
    @kind.setter
    def kind(self, value: GridCellKind):
        self._kind = value
...
class Grid(Building):
    ...
    """
    def __init__(self, y=-0.01, cell_size=(2.0, 2.0), row=7, col=7, grid_enabled=True):
        ...
        for i in range(row):
            for j in range(col):
                cell = GridCell(cell_size[0], cell_size[1], GridCellKind.Empty)
                cell.transform = Mat4.from_translation(
                    Vec3(
                        -self._w / 2 + cell_size[0] / 2.0 + cell_size[0] * j,
                        y,
                        -self._h / 2 + cell_size[1] / 2.0 + cell_size[1] * i
                    )
                )
                self.append(cell)
